...
An examination of the table above reveals that the reason for that huge difference in event types is caused by using different wording for the same kind of events in the raw data. 

For instance, you can find 83 variations for the official event type term _Thunderstorm Wind_ caused by:
...
This shows that in the majority of the cases the event reporters used the official event type names.
...
#### Step 2: Calculating the percentage that each event type contributes to the total amount of observations
